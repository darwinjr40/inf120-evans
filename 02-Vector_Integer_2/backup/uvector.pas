...
uses
  Classes, SysUtils, Forms, Controls, Graphics, Dialogs, Menus, StdCtrls,grids,Natural;
     ...
     type
       ...
       vector=class
         ...
         public                      //puntero
               ...
               //Retorna el elemeno mas repetido (moda)
   ...
   //-------------------UOTRAS TECNICAS--------------------------
               ...
               {---------EXAMEN-----------------------------}
               //divide el vector en ,pares,multiplos de 3 y palindrome
               ...
               //Hallar elemento mas repetido
               //function Ele_Moda():integer;
...
begin
  ...
  for i:=1 to dim-1 do begin
    for j:=i+1 to dim do begin
      n.setValor(elem[i]); n2.setValor(elem[j]);
      if(not n.Primo() and  n2.Primo())or
        (n.Primo() and n2.Primo() and(n.getValor()>n2.getValor()))or
      (not n.Primo() and not n2.Primo() and(n.getValor()>n2.getValor())) then
        InteElem(i,j);
    end;
  end;
end;
...
begin
  ...
  for i:=1 to dim do begin
    n.setValor(elem[i]);
    if(not n.Primo())then   //(ELEM[I]MOD 2==0) MULTIPLOS DE 2
     B.AddElem(n.getValor());    //Insertar no primos
   end;
   aux:=dim;    dim:=0;
  for i:=1 to aux do begin
    n.setValor(elem[i]);
    if( n.Primo())then  //(ELEM[I]MOD 3==0) MULTIPLOS DE 3
